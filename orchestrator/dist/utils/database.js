...
exports.INIT_SCHEMA = exports.DatabaseConfig = void 0;
...
class DatabaseConfig {
    static getPool() {
        if (!DatabaseConfig.instance) {
            throw new errors_1.AppError('Database pool not initialized. Call initialize() first.', 'DB_NOT_INITIALIZED');
        }
        return DatabaseConfig.instance;
    }
    static async initialize() {
        const { DATABASE_URL, DATABASE_POOL_MIN = '2', DATABASE_POOL_MAX = '20', DATABASE_TIMEOUT = '30000', } = process.env;
        if (!DATABASE_URL) {
            throw new errors_1.AppError('DATABASE_URL environment variable not set', 'ENV_VAR_MISSING');
        }
        ...
        try {
            ...
            await pool.end();
            throw new errors_1.AppError(`Failed to initialize database pool: ${error instanceof Error ? error.message : String(error)}`, 'DB_INIT_FAILED');
        }
...
async function initializeDatabase() {
    ...
    try {
        ...
        try {
            ...
            const statements = exports.INIT_SCHEMA.split(';')
                .map((stmt) => stmt.trim())

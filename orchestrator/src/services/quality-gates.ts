...
export class QualityGates {
   ...
   */
  private async validateTypeScript(): Promise<QualityCheck> {
    const startTime = Date.now();
   ...
   * Validate ESLint
   */
  private async validateESLint(): Promise<QualityCheck> {
    const startTime = Date.now();
   ...
   * Validate tests
   */
  private async validateTests(): Promise<QualityCheck> {
    const startTime = Date.now();
   ...
   */
  private async validateSecurity(): Promise<QualityCheck> {
    const startTime = Date.now();
   ...
   * Check code coverage
   */
  async checkCoverage(minCoverage: number = 80): Promise<QualityCheck> {
    const startTime = Date.now();

    try {
      const { stdout } = await execAsync(
        `cd ${this.projectPath} && npm test -- --coverage --passWithNoTests 2>&1 | grep -E "Lines|Functions|Branches" | head -1`,
      );

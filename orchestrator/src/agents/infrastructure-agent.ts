...
export interface InfrastructureTask {
  taskId: string;
  workflowId: string;
  infrastructure: string[]; // ['terraform', 'docker', 'cicd', 'monitoring']
...
export class InfrastructureAgent {
  ...
  constructor() {
    this.logger = new Logger('InfrastructureAgent');
   ...
   */
  async processInfrastructureGeneration(task: InfrastructureTask): Promise<GeneratedInfrastructure[]> {
    this.logger.info('Processing infrastructure task', {
      taskId: task.taskId,
      types: task.infrastructure,
    });

    const generated: GeneratedInfrastructure[] = [];
   ...
   */
  private async generateInfrastructure(task: InfrastructureTask, type: string): Promise<GeneratedInfrastructure> {
    ...
    try {
      ...
      const infra: GeneratedInfrastructure = {
        id: uuid().toString(),
        taskId: task.taskId,
        type: type as 'terraform' | 'docker' | 'cicd' | 'monitoring',
   ...
   */
  private buildInfrastructurePrompt(task: InfrastructureTask, type: string): string {
...
Include alert thresholds and notification channels.`;
   ...
   */
  private async validateInfrastructure(type: string, content: string): Promise<boolean> {
    try {
      switch (type) {
        case 'terraform':
   ...
   */
  private getOutputPath(type: string): string {
    switch (type) {
      case 'terraform':
   ...
   */
  private async storeInfrastructure(infra: GeneratedInfrastructure): Promise<void> {
    try {
      await DatabaseConfig.query(
        `INSERT INTO generated_infrastructure (id, task_id, type, content, path, validation_passed, generated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [infra.id, infra.taskId, infra.type, infra.content, infra.path, infra.validationPassed, infra.generatedAt]
      );
   ...
   */
  async getInfrastructure(taskId: string): Promise<GeneratedInfrastructure[]> {
    try {
      const result = await DatabaseConfig.query(
        `SELECT * FROM generated_infrastructure WHERE task_id = $1 ORDER BY generated_at DESC`,
        [taskId]
      );
